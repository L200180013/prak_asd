...
def cekInt(n):
    a = 0
    b = 0
    for i in n:#iterasi melalui baris
        for j in i:#iterasi melalui kolom
            b+=1
            if(str(j).isdigit()==False): #cek kolom
...
def ordo(n):
    a,b = 0,0
    for i in range(len(n)):
        a+=1 #tambah a sesuai jumlah baris
        b = len(n[i]) #hitung jumlah kolom
    print("mempunyai ordo "+str(a)+"x"+str(b))
...
def jumlah(n,m):
    ...
    if z==len(n) and z == len(m):
        print("ukuran sama")
        c =[[n[i][j] + m[i][j] for j in range
             (len(n[0]))] for i in range(len(n))]#membentuk matriks  
        print(c)
    else:
        print("ukuran beda")
...
def kali(n,m):
    ...
    if(y==v):
        ...
        for i in range(len(n)):#iterasi baris n
            for j in range(len(m[0])):#iterasi kolom m
                for k in range(len(m)):#iterasi baris m
